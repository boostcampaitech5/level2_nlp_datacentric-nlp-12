...
def preprocess_df(df: pd.DataFrame, h2h: bool = False, special: bool = False, jonghab: bool = False) -> pd.DataFrame:
    """
    주어진 데이터프레임을 전처리합니다.

    Parameters:
        df (pd.DataFrame): 전처리할 데이터프레임.
        h2h (bool, optional): 한자를 한글로 대체할지 여부를 나타내는 인자. 기본값은 False.
